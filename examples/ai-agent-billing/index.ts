...
async function ensureSecretKey(): Promise<{ key: string; claimUrl?: string; claimId?: string }> {
  ...
  process.env.INFI_SLUG = claimable.appSlug;
...
async function main() {
  ...
  // Register an identity app so `infi doctor` can pass after claim (slug from
  // claimable or a stable demo default). Origins from APP_URL / localhost.
  const appUrl = process.env.APP_URL ?? "http://localhost:3000";
  const appSlug = process.env.INFI_SLUG ?? "ai-agent-demo";
  process.env.INFI_SLUG ??= appSlug;
  const companyToSync = withAppUrl(
    {
      ...company,
      apps: [
        {
